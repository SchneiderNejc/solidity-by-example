...
/* Deployment order:
attacker -> DeployerDeployer
attacker -> DeployerDeployer ->(C2) Deployer
attacker -> DeployerDeployer ->(C2) Deployer -> Proposal
attacker -> DeployerDeployer
attacker -> DeployerDeployer ->(C2) Deployer
attacker -> DeployerDeployer ->(C2) Deployer -> (C2) Attack

DAO contract then invokes Attack, since it is now at the same address as Proposal used to be.

DAO contracts needs to use delegatecall in order to exploit the volunrability.
We can then change any state variable (as long as we are familiar with storage layout). */
